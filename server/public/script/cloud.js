...
document.addEventListener("DOMContentLoaded", async function () {
  try {
    const path = sessionStorage.getItem("path") || "root";
...
const addFileToList = (file) => {
  ...
  if (file.type === "folder") {
    ...
    fileButtons.classList.add("dir-buttons");
...
const loadPath = (path) => {
  ...
  pathText.innerHTML = "";
  pathText.appendChild(homeButton);
  //* Check if path is root
  if (path === "root") return;
  //* Create path elements
  const separator = document.createElement("span");
  separator.innerHTML = "/";
  pathText.appendChild(separator);
  const pathArray = path.split("/");
  pathArray.forEach((dir, index) => {
    const pathItem = document.createElement("span");
    pathItem.innerHTML = dir;
    if (index === pathArray.length - 1) return pathText.appendChild(pathItem);
    pathText.appendChild(pathItem);
    const separator = document.createElement("span");
    separator.innerHTML = "/";
    pathText.appendChild(separator);
  });
...
const changeDirectory = () => {
  //* Get all directories
  const dirs = document.querySelectorAll(".files-list-item.dir");
  ...
  dirs.forEach((dir) => {
    ...
    dir.addEventListener("click", async (e) => {
      ...
      const path = e.target.closest(".files-list-item.dir").dataset.name;
  ...
  shareBtn.forEach((btn) => {
    ...
    newButton.addEventListener("click", async (e) => {
      ...
      if (res.status === 200) {
        ...
        //* Copy to clipboard
        navigator.clipboard.writeText(data.shareUri);
